...
@Component({
  selector: 'app-specilized-firm-details',
  templateUrl: './specilized-firm-details.component.html',
  styleUrls: ['./specilized-firm-details.component.scss']
})
export class SpecilizedFirmDetailsComponent {
  ...
  searchQuery: any;
    ...
    this.isLoading = true;
    this.service.getSpecilizedFirm().subscribe(
        (response: any[]) => {
           const filteredResponse = response.filter(
            ...
            this.tableData = filteredResponse.map(item => ({
                id: Number(item.id) || 0,
                bcta_no: item.specializedFirmNo || '-',  // Changed from contractorNo to specializedFirmNo
        ...
        (error) => {
            this.isLoading = false;
            console.error('Error fetching specialized firms:', error);
        }
    );
}
...
navigate(FirmName: string) {
  this.service.setData(
    { data: FirmName },  // e.g. "ABC Construction"
    'FirmName',
    'monitoring/ViewSpecializeDetails'
  );
}
      ...
      SaveAndForward(): void {
             ...
             this.service.specilizedFirmNotifyingMonitoringCommittee(this.selectedApplicationNo).subscribe({
               next: (response: any) => {
