...
@Component({
  selector: 'app-consultency-firm-details',
  templateUrl: './consultency-firm-details.component.html',
  styleUrls: ['./consultency-firm-details.component.scss']
})
export class ConsultencyFirmDetailsComponent {
  ...
  searchQuery: any;
    ...
    this.isLoading = true;
    this.service.getConsultancyFirm().subscribe(
        (response: any[]) => {

            const filteredResponse = response.filter(
                item => item.applicationStatus === 'In Process'
            );
            // Map the filtered response to tableData
            this.tableData = filteredResponse.map(item => ({
                id: Number(item.id) || 0,
                bcta_no: item.consultantNo || '-',  // Changed from contractorNo to consultantNo
                ...
                license_status: this.determineLicenseStatus(item.applicationStatus), // Added license status mapping
        ...
        (error) => {
            this.isLoading = false;
            console.error('Error fetching consultancy firms:', error);
        }
    );
}
     ...
     SaveAndForward(): void {
       ...
       this.service.consultantFirmNotifyingMonitoringCommittee(this.selectedApplicationNo).subscribe({
         next: (response: any) => {
