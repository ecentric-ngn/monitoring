...
export class ContractDocumentComponent {
  @Output() onContractorDocSaved = new EventEmitter<{ tableId: any; data: any;inspectionType:any }>();
  ...
  ngOnInit() {
   ...
   getDatabasedOnChecklistId() {
      const payload : any =[ {
          ...
          this.service.fetchDetails(payload,1,2,'contract_document_at_site').subscribe(
            (response: any) => {
              const data = response.data[0];
              this.formData.ConstructionPermit= data.construction_permit_available;
              this.formData.contractConditions= data.construction_conditions_complied;
              this.formData.approvedDrawings= data.approved_drawings_available;
              this.formData.workInsurance= data.workers_insurance_provided;
              this.formData.workInsuranceStatus= data.work_insurance_status;
              this.formData.thirdPartyInsurance= data.third_party_insurance_status;
              this.formData.billofQuantities= data.bill_of_quantities_available;
              this.formData.materialsQualityCertification= data.materials_quality_certified;
              this.formData.qualityAssurancePlan= data.quality_assurance_plan_available,
              this.formData.qualityControlPlan= data.quality_control_plan_available,
              this.formData.TestconductedasperQAP= data.tests_conducted_as_per_standards,
