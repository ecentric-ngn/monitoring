...
@Component({
  selector: 'app-consultant-comments-adverse-records',
  templateUrl: './consultant-comments-adverse-records.component.html',
  styleUrls: ['./consultant-comments-adverse-records.component.scss'],
})
export class ConsultantCommentsAdverseRecordsComponent {
  @Input() data: any;
...
//methos to get the record list
  getRecordList(data: any) {
    const consultant = {
      viewName: 'consultantAdverseCommentRecord',
      pageSize: this.pageSize,
      pageNo: this.pageNo,
      condition: [{ field: 'consultantNo', value: data.consultantNo }],
    };
    this.loading=true
...
//method to save the comment
  saveComment(){
    ...
    const commentRecord = {
      ...
      consultantNo: this.data.consultantNo,
    };
    this.loading=true
...
//method to save the adverse record
  savedAdverseList() {
     if (this.formData.Date) {
      // Parse the selected date
      const selectedDate = new Date(this.formData.Date);
...
//methos to save the monitoring list
  savedMonitoringList(){
     if (this.formData.Date) {
      // Parse the selected date
      const selectedDate = new Date(this.formData.Date);
