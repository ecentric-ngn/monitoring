...
@Component({
  selector: 'app-deregister-specialized-firm',
  templateUrl: './deregister-specialized-firm.component.html',
  styleUrls: ['./deregister-specialized-firm.component.scss']
})
export class DeregisterSpecializedFirmComponent {
  searchQuery: any;
...
navigate(data:any){
  const title = 'List Of Deregistered Specialized Firm'
...
SearchFilter() {
  if (this.searchQuery && this.searchQuery.trim() !== '') {
    this.getDeregisteredList(this.searchQuery); // Pass search query to backend
  } else {
    this.getDeregisteredList(); // Fetch all data
  }
}
showErrorMessage:boolean=false
//Getting the consultant list for displaying in the table
getDeregisteredList(searchQuery?: string){
...
this.reregisterDeregisteredSpecializedFirm();
}
...
//save deregister
reregisterDeregisteredSpecializedFirm() {
if (this.formData.Date) {
  ...
  const deregisterSpecializedFirm = {
    reregisterDate: this.formData.Date,
    reregisterDetails: this.formData.Details,
    reregisterBy: this.uuid,
    specializedFirmNo: this.selectedspecializedFirmNo,
    fileId:this.fileId
    };
    this.service.saveDeReregister(deregisterSpecializedFirm).subscribe((response: any) => {
        setTimeout(() => {
