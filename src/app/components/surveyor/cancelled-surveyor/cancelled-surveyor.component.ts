...
@Component({
  selector: "app-cancelled-surveyor",
  templateUrl: "./cancelled-surveyor.component.html",
  styleUrls: ["./cancelled-surveyor.component.scss"],
})
export class CancelledSurveyorComponent {
...
shouldShowActionButton(): boolean {
  ...
  navigate(data: any) {
    const title = 'List Of Cancelled Surveyor'
  ...
  SaveSurveyor(actionForm) {
    ...
    this.reregisterCancelledSurveyor();
  }
  ...
  reregisterCancelledSurveyor(){
    ...
    this.service.saveCancelledReregister(CancelledSurveyor).subscribe(
      (response: any) => {
        setTimeout(() => {
  ...
  showReinstateMessage() {
    this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Surveyor reregistered successfully' });
  }
