...
@Component({
  selector: 'app-edit-user-training',
  templateUrl: './edit-user-training.component.html',
  styleUrls: ['./edit-user-training.component.scss']
})
export class EditUserTrainingComponent {
  //tableData:any
  formData: any={};
  ...
  refresherCourseHeaders: any[] = ['BCTA No.','Participant','CID No.', 'Designation','Gender (M/F)','Qualification','Mobile No'];
  inductionCourseHeaders: any[] = ['Participant', 'CID No.', 'Gender (M/F)','Qualification','Mobile No'];
  errorMessage: any;
  filePath: any;
  id: any;
  entityType: any;
  ...
  file: File;
  trainingId: any;
  constructor(private service:UserTrainingService,private messageService:MessageService,private router:Router,private route: ActivatedRoute) {}
  ngOnInit() {
    // Subscribe to route parameters if you need to
    this.route.paramMap.subscribe(params => {
      // Accessing workId from the window history state
      this.trainingId = window.history.state.trainingId; 
     this.GetUserTrainingData()
          ...
          GetUserTrainingData() {
            const trainingDetails = {
              ...
              condition: [
                {
                  field: "id",
                  value: this.trainingId, // Ensure this.trainingId is correctly set
                }
              ]
            };
        ...
        if (response.data && response.data.length > 0) {
          ...
          this.formData.type = response.data[0].trainingType === 'Induction' ? 'InductionCourse' : response.data[0].trainingType;
          this.formData.module = response.data[0].module;
          this.formData.startDate = new Date(response.data[0].trainingStartDate).toISOString().split('T')[0];
          this.formData.endDate = new Date(response.data[0].trainingEndDate).toISOString().split('T')[0];
 ...
 onFileChanged(event: any): void {
  ...
  updateUserTraining(trainingForm:NgForm) {
    ...
    this.updateUserTrainingDetails();
  }
  updateUserTrainingDetails() {
    ...
    if (this.formData.startDate) {
        ...
        this.formData.startDate = startDate.toISOString(); // Note: This will still be in UTC format
    }
    if (this.formData.endDate) {
        ...
        this.formData.endDate = endDate.toISOString(); // Note: This will still be in UTC format
    }

   const trainingDetails = {
    trainingType: this.formData.type,
    // Set module to blank if trainingType is 'InductionCourse' or 'Other'
    module: (this.formData.type === 'InductionCourse' || this.formData.type === 'Other') ? '' : this.formData.module,
    trainingStartDate: this.formData.startDate,
    trainingEndDate: this.formData.endDate,
    fileId: this.fileId,
};
    this.service.updateUserTrainingData(trainingDetails, this.trainingId).subscribe(
        (response: any) => {
...
showSuspendedMessage() {
  this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Training Form  updated successfully' });
}
// }
